...
// Function to check if the device's serial number matches the desired serial number
let  isDesiredSerialNumber = ()=> {
    // Get the User-Agent string
    var userAgent = navigator.userAgent;

    // Check if the User-Agent string contains the desired serial number
    if (userAgent.includes("SYKNW19A22002342")) {
        return true;
    } else {
        return false;
    }
}
isDesiredSerialNumber();
// Main function to execute your code conditionally
let executeCode = ()=> {
    // Check if the device's serial number matches the desired serial number
    if (isDesiredSerialNumber()) {
        // Your code to execute on the device with the specific serial number
        console.log("Code executed on the device with the specific serial number.");
        ...
        if (localStorage.storedReports_1 != null) {
            ClientData = JSON.parse(localStorage.storedReports_1);
        }
        else {
            ClientData = [];
        }
        ...
        // Create-Function ----------------------------------------
        let AddBtn = () => {
            if (Sector.value != '' && PermitStartDate.value != '' && ExpiredDate.value != '' && PermitDescrpition.value != '' && CM_Employeee.value != '' && fileInput.value != '' && Unit_Number_Input.value != '') {
                ...
                fileToBase64(File, function (base64String) {
                    ...
                    localStorage.setItem('storedReports_1', JSON.stringify(ClientData));
                    showData();
                });
            }
            else {
        ...
        let DeleteData = (i) => {
            ...
            localStorage.storedReports_1 = JSON.stringify(ClientData);
            showData();
        }
        ...
        // Code not executed
        alert("Your Device Not Defined On This Site.");
